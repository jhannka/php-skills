...
- **Rule 6**: `Log::error(['message' => $e->getMessage()])` is the safe, established pattern (server-side only) — but `Log::error(['trace' => $e->getTraceAsString()])` is NOT the same thing and is a real finding, not a false positive. `getTraceAsString()` includes, by default, the arguments passed to every frame of the call stack, which can leak tokens/PII passed in lower layers even though the log itself is server-side (it can still reach log aggregators, error trackers, or support tickets pasted from logs). Fix: use `ExceptionHandler::safeError($e, $msg)` (already used in `VacancyController`), which logs message/file/line/code but never the full trace. Hit via PR 2287 (`ApplicationPublicController`), 2026-08-26.
...
- **Rule 5** (PostgreSQL): `ALTER TABLE ... ALTER COLUMN ... TYPE ...` does NOT drop an existing `CHECK` constraint on that column. Changing a column's type (e.g. widening an enum-backed `varchar` to accept new values) while a `CHECK (col IN (...))` from the original column definition is still active will silently keep rejecting the new values, defeating the migration's purpose. Explicitly `ALTER TABLE x DROP CONSTRAINT IF EXISTS x_col_check` before the `ALTER COLUMN TYPE`, and again (defensively, `IF EXISTS`) before re-adding a constraint in `down()`. Hit via PR 2277 (`alter_empleados_sexo_to_varchar`), 2026-08-27.
- **Rule 6**: `$table->enum('col', [...])` never creates a native PostgreSQL enum type — Laravel's `PostgresGrammar::typeEnum()` always emits `varchar(255) CHECK (col IN (...))` as part of the column definition, on every version this project has used. Don't write a `down()` that assumes a "custom enum type" exists to reference/drop; the correct rollback is to normalize any values outside the original enum's list (they won't fit the restored `CHECK`) and then restore the exact `varchar(255) + CHECK` definition from the original migration. Confirmed via `vendor/laravel/framework/.../Schema/Grammars/PostgresGrammar.php` and the original `create_empleados_table` migration. Hit via PR 2295 (`alter_empleados_sexo_to_varchar` down()), 2026-08-26.
